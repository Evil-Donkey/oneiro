    ...
    return (
        <div className={`${styles.slidingBlocks} text-white`} ref={containerRef}>
            <div className='container mx-auto relative px-6 md:px-4'>
                ...
                {blocks && 
                    <div className={`mt-30 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-20`} ref={blocksRef}>
                        {blocks.map((item, index) => {
                            const { heading, copy, icon } = item;
                            return (
                                <div key={index} className={`${styles.slidingBlockItem} flex flex-col gap-4 p-8 bg-blue-05 rounded-lg`}>
